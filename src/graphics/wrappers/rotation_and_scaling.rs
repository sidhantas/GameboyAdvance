        ...
        Some(Self([
            [
                fixed88_point_to_floating_point(u16::from_le_bytes(
                    memory.oam[(group * 0x20 + 0x6)..][..2].try_into().unwrap(),
                )),
                fixed88_point_to_floating_point(u16::from_le_bytes(
                    memory.oam[(group * 0x20 + 0xe)..][..2].try_into().unwrap(),
                )),
            ],
            [
                fixed88_point_to_floating_point(u16::from_le_bytes(
                    memory.oam[(group * 0x20 + 0x16)..][..2].try_into().unwrap(),
                )),
                fixed88_point_to_floating_point(u16::from_le_bytes(
                    memory.oam[(group * 0x20 + 0x1e)..][..2].try_into().unwrap(),
                )),
            ],
        ]))
    }
...
mod affine_tests {
    ...
    fn try_transform() {
        ...
        memory.oam[0x26] = 0xb8;
        memory.oam[0x3e] = 0xb8;
